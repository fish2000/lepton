...
namespace IOUtil {
	...
	*/ /*
	...
	void send_all_and_close(int fd, const uint8_t* data, std::size_t data_size) {
	  while (data_size > 0) {
	...
	Sirikata::Array1d<uint8_t, 16> send_and_md5_result(const uint8_t* data,
	  ...
	  *output_size = 0;
	...
	#ifdef _WIN32
	  std::thread send_all_thread(std::bind(&send_all_and_close,
	  										 send_to_subprocess,
											 data, data_size));
	  FILE* fp = _fdopen(recv_from_subprocess, "rb");
	  while (true) {
	    auto ret = std::fread(buffer, 1, sizeof(buffer), fp);
	    if (ret == 0) {
	      break;
	    }
	    *output_size += ret;
	  ...
	  while (!finished) {
	    ...
	    ret = ::select(max_fd, &readfds, &writefds, &errorfds, nullptr);
	#endif
	    ...
	    if (recv_from_subprocess != -1
	#ifndef EMSCRIPTEN
	        && (FD_ISSET(recv_from_subprocess, &readfds) ||
	            FD_ISSET(recv_from_subprocess, &errorfds))
	#endif
	            ) {
	    ...
	    if (send_to_subprocess != -1
	#ifndef EMSCRIPTEN
	        && (FD_ISSET(send_to_subprocess, &writefds) ||
	            FD_ISSET(send_to_subprocess, &errorfds))
	#endif
	            ) {
	      ssize_t del = 0;
	      if (send_cursor < data_size) {
	        del = ::write(send_to_subprocess, &data[send_cursor],
	                    data_size - send_cursor);
	      }
	      // std::fprintf(stderr, "Want %ld bytes, but sent %ld\n", data_size -
	      // send_cursor,  del);
	      if (del < 0 && (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK)) {
	        ...
	        send_cursor += del;
	        // std::fprintf(stderr, "len Storage is %ld/%ld\n", send_cursor, data_size);
	      }
	...
	void md5_and_copy_to_tee(int copy_to_input_tee, int input_tee, MD5_CTX *context,
	                         std::size_t* input_size, std::size_t start_byte, std::size_t end_byte,
	                         bool close_input, bool is_socket) {
	  unsigned char buffer[65536];
	  while (true) {
	    std::size_t max_to_read = sizeof(buffer);
	    ...
	    if (del == 0) {
	      ...
	      if (end_byte != 0 && *input_size + del > end_byte &&
	          end_byte > *input_size) {
	      ...
	      if (*input_size + del > start_byte) {
	        if (*input_size >= start_byte) {
	          ...
	          std::size_t offset = (start_byte - *input_size);
	      ...
	      *input_size += del;
	      ...
	      // std::fprintf(stderr,"%d) retry Err\n", copy_to_input_tee);
	    } else {
	...
	Sirikata::Array1d<uint8_t, 16> transfer_and_md5(Sirikata::Array1d<uint8_t, 2> header, std::size_t start_byte,
	                 								std::size_t end_byte, bool send_header, int copy_to_input_tee,
	                 							    int input_tee, int copy_to_storage, std::size_t* input_size,
	  ...
	  *input_size = header.size();
	...
	#ifdef _WIN32
	  std::thread worker(std::bind(&md5_and_copy_to_tee, copy_to_input_tee,
	                               input_tee, &context, input_size, start_byte,
	                               end_byte, close_input, is_socket));
	#if 1
	  while (true) {
	    ...
	    auto delta = ::read(copy_to_storage, storage->data() + old_size,
	  ...
	  while (true) {
	    ...
	    storage->resize(storage->how_much_reserved());
	    auto ret = std::fread(storage->data() + old_size, 1, storage->size() - old_size, fp);
	  ...
	  static_assert(sizeof(buffer) >= header.size(),
	                "Buffer must be able to hold header");
	  uint32_t cursor = 0;
	  bool finished = false;
	  while (!finished) {
	#ifndef EMSCRIPTEN
	    ...
	    // std::fprintf(stderr, "START POLL %d\n", max_fd);
	    int ret = 1;
	#ifndef EMSCRIPTEN
	    ret = ::select(max_fd, &readfds, &writefds, &errorfds, nullptr);
	#endif
	    // std::fprintf(stderr, "FIN POLL %d\n", ret);
	    if (ret == 0 || (ret < 0 && errno == EINTR)) {
	      continue;
	    }
	    // std::fprintf(stderr, "Checking ev\n");
	    if (copy_to_input_tee != -1 && cursor < sizeof(buffer)
	#ifndef EMSCRIPTEN
	        && (FD_ISSET(copy_to_input_tee, &readfds) ||
	            FD_ISSET(copy_to_input_tee, &errorfds))
	#endif
	            ) {
	      always_assert(cursor < sizeof(buffer)); // precondition to being in the poll
	      std::size_t max_to_read = sizeof(buffer) - cursor;
	      if (end_byte != 0 && max_to_read > end_byte - *input_size) {
	        max_to_read = end_byte - *input_size;
	      }
	      ...
	      if (del == 0) {
	        ...
	        // std::fprintf(stderr,"input:Should close(%d) size:%ld\n",
	      ...
	      // std::fprintf(stderr,"%d) Reading %ld bytes for total of %ld\n",
	      // copy_to_input_tee, del, *input_size);
	    }
	    if (copy_to_storage != -1
	#ifndef EMSCRIPTEN
	        && (FD_ISSET(copy_to_storage, &readfds) ||
	            FD_ISSET(copy_to_storage, &errorfds))
	#endif
	            ) {
	      ...
	      ssize_t del = ::read(copy_to_storage,
		  							&(*storage)[old_size],
	                         		storage->size() - old_size);
	      // std::fprintf(stderr, "Want %ld bytes, but read %ld\n", storage->size() -
	      // old_size,  del);
	      if (del < 0 &&
	          (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK)) {
	        // std::fprintf(stderr, "EAGAIN %d\n", errno);
	        storage->resize(old_size);
	      } else if (del < 0) {
	        // std::fprintf(stderr, "Error %d\n", errno);
	        break;
	        ...
	        if (del == 0) {
	          storage->resize(old_size);
	          finished = true;
	          // std::fprintf(stderr,"back_copy:Should close(%d):size
	        ...
	        if (del > 0) {
	          ...
	          // std::fprintf(stderr, "len Storage is %ld\n", storage->size());
	        }
	      }
	    }
	    if (input_tee != -1 && cursor != 0
	      ...
	      // std::fprintf(stderr, "fd: %d: Writing %ld data to %d\n", input_tee, del,
	      // cursor);
	      // std::fprintf(stderr, "A");
	      if (del == 0) {
	        // std::fprintf(stderr, "B\n");
	        while (::close(input_tee) < 0 && errno == EINTR) {}
	        // std::fprintf(stderr,"output_to_compressor:Should close (%d) \n", input_tee
	        // );
	        input_tee = -1;
	      }
	      ...
	      if (del > 0) {
	        ...
	        if (del < cursor) {
	          // std::fprintf(stderr, "E %ld %ld\n", del, cursor - del);
	          std::memmove(buffer, buffer + del, cursor - del);
	        }
	        cursor -= del;
	      }
	    }
	...
	SubprocessConnection start_subprocess(int argc, const char** argv,
	                                      bool pipe_stderr) {
	  ...
	  std::memset(&piProcInfo, 0, sizeof(piProcInfo));
	  ...
	  command_line.insert(command_line.end(), exe_shorthand,
	                      exe_shorthand + std::strlen(exe_shorthand));
	  for (int i = 1; i < argc; ++i) {
	    command_line.push_back(' ');
	    command_line.insert(command_line.end(), argv[i], argv[i] + std::strlen(argv[i]));
	  }
	  ...
	  if (!CreateProcess(argv[0], &command_line[0],
	                     ...
	                     nullptr, // use current dir,
	                     &siStartInfo, &piProcInfo)) {
	    std::fprintf(stderr, "Failed To start subprocess with command line ",
	            command_line);
	    custom_exit(ExitCode::OS_ERROR);
	  }
	  ...
	  if (pipe_stderr || !simpler) {
	    ...
	    while ((retval.pipe_stderr = _open_osfhandle((std::intptr_t)hChildStd_ERR_Rd,
	                                                 O_APPEND | O_RDONLY)) == -1 &&
	           errno == EINTR) {}
	  } else {
	  ...
	  while ((retval.pipe_stdout = _open_osfhandle((std::intptr_t)hChildStd_OUT_Rd,
	                                               O_APPEND | O_RDONLY)) == -1 &&
	         errno == EINTR) {}
	  CloseHandle(hChildStd_IN_Rd);
	  while ((retval.pipe_stdin = _open_osfhandle((std::intptr_t)hChildStd_IN_Wr,
	                                              O_APPEND | O_WRONLY)) == -1 &&
	         errno == EINTR) {}
	#else
