...
namespace Sirikata {
	...
	class SIRIKATA_EXPORT BoundedMemWriter : public Sirikata::DecoderWriter {
	    ...
	    std::size_t mNumBytesAttemptedToWrite;
		
		public:
		    
			BoundedMemWriter(const JpegAllocator<uint8_t> &alloc = JpegAllocator<uint8_t>())
				:mBuffer(alloc)
				{
		        	mWriteCursor = 0;
			...
			void set_bound(std::size_t bound) {
			...
			virtual std::pair<Sirikata::uint32, Sirikata::JpegError> Write(const Sirikata::uint8* data,
		        ...
		        if (mBuffer.size() > mWriteCursor) {
		            bounded_size = (unsigned int)std::min((std::size_t)size, mBuffer.size() - mWriteCursor);
		            std::memcpy(&mBuffer[mWriteCursor], data, bounded_size);
		        }
		    ...
		    MuxReader::ResizableByteBuffer const& buffer() const {
		        return mBuffer;
		    }
