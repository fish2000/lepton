...
struct MBlockContext {
	...
	constexpr ABlock const& here() const {
        return cur[0];
    }
    
	constexpr ABlock const& left_unchecked() const {
        return cur[-1];
    }
    
	constexpr ABlock const& above_unchecked() const {
        return above[0];
    }
    
	constexpr ABlock const& above_left_unchecked() const {
        return above[-1];
    }
	...
	NeighborSummary const& neighbor_context_above_unchecked() const {
        // too slow // assert(num_nonzeros_check(*num_nonzeros_above, above_unchecked()));
        return *num_nonzeros_above;
    }
    
	NeighborSummary const& neighbor_context_left_unchecked() const {
        std::vector<NeighborSummary>::iterator tmp = num_nonzeros_here;
        --tmp;
        // too slow // assert(num_nonzeros_check(*tmp, left_unchecked()));
        return *tmp;
