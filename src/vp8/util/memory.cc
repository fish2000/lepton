...
extern "C" {
	void always_assert_exit(bool value, const char* expr, const char* file, int line) {
	    //if (!value) {
	        while (::write(2, "Assert Failed: ", std::strlen("Assert Failed: ")) < 0 && errno == EINTR) {}
	        while (::write(2, expr, std::strlen(expr)) < 0 && errno == EINTR) {}
	        while (::write(2, " at (", 5) < 0 && errno == EINTR) {}
	        while (::write(2, file, std::strlen(file)) < 0 && errno == EINTR) {}
	...
	void* custom_realloc (void* old, std::size_t size) {
	    ...
	    void* retval = Sirikata::MemMgrAllocatorRealloc(old, size, &actual_size, true, nullptr);
	    if (retval == 0) { // did malloc succeed?
	        if (!g_use_seccomp) {
	            assert(false && "Out of memory error");
	        }
...
void custom_exit(ExitCode exit_code) {
    ...
    if (exit_code != ExitCode::SUCCESS) {
        while (::write(2, ExitString(exit_code), std::strlen(ExitString(exit_code))) < 0 && errno == EINTR) {}
        while (::write(2, "\n", 1) < 0 && errno == EINTR) {}
    }
#ifdef __linux
