...
struct IdentityArray1d {
    ...
    template <int a>
	static constexpr uint8_t kat() { return a; }
};
	...
	public:
		...
		uint8_t recalculate_coded_length() const {
			...
			for (unsigned int idx = 0; idx < 64; idx++) {
				unsigned int xy = jpeg_zigzag_to_raster.at(idx);
	        	unsigned int x = xy & 7;
	        	unsigned int y = xy >> 3;
	  	...
	  	int16_t& mutable_coefficients_raster(uint8_t idx) {		return coef.at(raster_to_aligned.at(idx)); }
	  	int16_t  coefficients_raster(uint8_t idx) const { 		return coef.at(raster_to_aligned.at(idx)); }

	  	int16_t& mutable_coefficients_zigzag(uint8_t idx) {		return coef.at(zigzag_to_aligned.at(idx)); }
	  	int16_t  coefficients_zigzag(uint8_t idx) const { 		return coef.at(zigzag_to_aligned.at(idx)); }
