...
class ThreadHandoff {
	...
	public:
	    ...
	    bool is_legacy_mode() const {
			// legacy mode doesn't have access to handoff data
	        return num_overhang_bits == LEGACY_OVERHANG_BITS;
	    }
		
	    static std::size_t get_remaining_data_size_from_two_bytes(unsigned char input[2]);
	    static std::vector<ThreadHandoff> deserialize(const unsigned char* data, std::size_t max_size);
	    ...
	    ThreadHandoff operator-(ThreadHandoff const& other) const;
};

#endif /// THREAD_HANDOFF_HH_
