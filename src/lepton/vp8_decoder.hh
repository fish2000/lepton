...
class VP8ComponentDecoder : public BaseDecoder, public VP8ComponentEncoder {
	...
	VP8ComponentDecoder(VP8ComponentDecoder const&) = delete;
    VP8ComponentDecoder& operator=(VP8ComponentDecoder const&) = delete;
	...
	public:
	    ...
	    template <bool force_memory_optimized>
	    std::vector<ThreadHandoff> initialize_decoder_state(const UncompressedComponents* const colldata,
	    ...
	    std::size_t get_model_memory_usage() const {
	        return model_memory_used();
	    }
		
	    std::size_t get_model_worker_memory_usage() const {
	        return model_worker_memory_used();
	    }
	    
		~VP8ComponentDecoder();
	    
		void initialize(Sirikata::DecoderReader* input, std::vector<ThreadHandoff> const& thread_transition_info);
