...
class LeptonCodec {
	...
	protected:
	    ...
	    std::size_t model_worker_memory_used() const {
	        std::size_t retval = 0;
	        for (std::size_t i = 1;i < thread_state_.size(); ++i) {
	            if (thread_state_[i]) {
	    ...
	    std::size_t model_memory_used() const {
	        std::size_t retval = 0;
	        for (std::size_t i = 0;i < thread_state_.size(); ++i) {
	            if (thread_state_[i]) {
	                retval += sizeof(ProbabilityTablesBase);
	            }
	        }
